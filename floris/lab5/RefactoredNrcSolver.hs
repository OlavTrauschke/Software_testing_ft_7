module Lab5RefactoredNrcSolver where 
  ...
  -- If there is no nrc subgrid we'll say all values are free
  freeInSubgridNrc :: Sudoku -> (Row,Column) -> [Value]
  freeInSubgridNrc s (r,c)
    | sg == []   = values
    | otherwise  = freeInSeq sg
    where sg = subGridNrc s (r,c)
