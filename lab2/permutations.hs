...
--Lazy implementation of isPermutation by Joryd
isPermutation' :: Eq a => [a] -> [a] -> Bool
isPermutation' xs ys = elem ys (permutations xs)

--Implementation of isPermutation without using permutations by Areg
isPermutation :: Eq a => [a] -> [a] -> Bool
isPermutation [] [] = True
isPermutation (x:xs) ys = (elem x ys) && (isPermutation xs (delete x ys))
isPermutation _ _ = False

--Tests (by Olav) below, usage: testRepeated 1 [number of tests] isPermutation or isPermutation'
