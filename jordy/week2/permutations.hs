...
randomPermutation xs = do 
   b <- getRandomInt 1
   if b==0 then getRandomElement (fmap permutations xs) else genIntList'

-- Choose a random element from a list (currently not really functioning (index out of bound))
getRandomElement :: IO [[a]] -> IO [a]
